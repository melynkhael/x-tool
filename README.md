...
- Parse the **entire** archive (`tweet.js` / `tweets.js`), no 3200-tweet
  scroll limit.
...
### `xtool parse <tweet.js> [-o out.jsonl]`

Reads the `window.YTD.tweets.partN = [ ... ]` JavaScript assignment,
strips the prefix, parses the JSON array, flattens each
`{ "tweet": {...} }` wrapper, and writes one tweet per line.
...
| `--dry-run`          | don't actually delete, just print             |
| `--log FILE`         | progress log (default `deleted.jsonl`)        |
